...
const register = async (userData) => {
  ...
  const response = await axios.post(
    `${API_URL}/api/auth/store/register`,
    userData,
  );
...
const loginReseller = async (resellerCode) => {
  const response = await axios.post(`${API_AUTH_URL}/reseller-login`, {
    resellerCode,
  });
...
const forgotPassword = async (emailData) => {
  const response = await axios.post(
    `${API_URL}/api/auth/forgot-password-store`,
    emailData,
  );
  return response.data;
};
