...
const PaymentRedirectPage = () => {
  ...
  const mainGradient =
    "linear-gradient(135deg, rgba(49, 0, 138, 0.85) 0%, rgba(49, 0, 138, 0.85) 35%, rgba(168, 85, 247, 0.85) 65%, rgba(247, 37, 133, 0.85) 100%)";
  ...
  useEffect(() => {
    ...
    const handleFinalizePayment = async () => {
      try {
        const response = await axios.get(
          `${API_URL}/api/orders/cart/payment-redirect-handler${location.search}`,
        );
    ...
    if (code === "1") {
      ...
      setStatus("error");
      setMessage(
        `La transacción fue rechazada. Motivo: ${description || "Desconocido"}. Por favor, intente de nuevo.`,
      );
    }
        ...
        <Box sx={fadeInUp}>
          <Card sx={glassStyle}>
            <CardContent sx={{ p: { xs: 4, md: 6 }, textAlign: "center" }}>
              ...
              >
                Su pedido ha sido procesado y el pago ha sido aprobado. ¡Gracias
        ...
        <Box sx={fadeInUp}>
          <Card sx={glassStyle}>
            <CardContent sx={{ p: { xs: 4, md: 6 }, textAlign: "center" }}>
              ...
              <Typography
                variant="h3"
                sx={{ fontWeight: 900, color: "white", mb: 2 }}
              >
                ¡Pago No Procesado!
              </Typography>
              <Typography
