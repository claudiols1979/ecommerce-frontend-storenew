      ...
      <Box sx={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: mainGradient, p: 3 }}>
        <Card sx={glassStyle}>
          <CardContent sx={{ p: 4, textAlign: "center" }}>
            <Alert severity="error" sx={{ borderRadius: "16px", mb: 2 }}>{error.message}</Alert>
            <Button variant="contained" onClick={() => window.location.reload()} sx={{ borderRadius: "12px", textTransform: "none", fontWeight: "bold" }}> Reintentar </Button>
          </CardContent>
        </Card>
      </Box>
    );
  ...
  return (
    <Box sx={{
      ...
      <Container maxWidth="lg" sx={{ position: "relative", zIndex: 1 }}>
        ...
        {user?.isBlocked && (
          <Alert severity="error" className="animate-fade-in-up" sx={{ mb: 4, borderRadius: "16px", background: "rgba(211, 47, 47, 0.2)", color: "#ffcdd2", border: "1px solid rgba(211, 47, 47, 0.3)" }}>
        ...
        {cartItems.length === 0 ? (
          <Card className="animate-fade-in-up" sx={{ ...glassStyle, p: { xs: 4, sm: 8 }, textAlign: 'center' }}>
            <ShoppingBagIcon sx={{ fontSize: 80, mb: 3, opacity: 0.5 }} />
            <Typography variant="h5" sx={{ mb: 4, fontWeight: 600 }}>Tu carrito está vacío</Typography>
            <Button
              variant="contained"
          ...
          <Grid container spacing={4}>
                    ...
                    return (
                      <Box
                        ...
                        sx={{
                          ...
                          borderBottom: index === cartItems.length - 1 ? 'none' : '1px solid rgba(255,255,255,0.1)',
                          gap: { xs: 2, sm: 3 },
                        }}
                      >
                        <Box
                        ...
                        <Box sx={{ flexGrow: 1, textAlign: { xs: 'center', sm: 'left' } }}>
                          <Typography variant="h6" sx={{ fontWeight: 800, mb: 0.5, letterSpacing: "-0.01em" }}>{item.name}</Typography>
                          <Box sx={{ display: 'flex', gap: 2, justifyContent: { xs: 'center', sm: 'flex-start' }, mb: 1, opacity: 0.7 }}>
                            <Typography variant="caption" sx={{ display: "flex", alignItems: "center" }}>
                              <CodeIcon sx={{ fontSize: 14, mr: 0.5 }} /> {item.code}
                            </Typography>
                            <Typography variant="caption" sx={{ display: "flex", alignItems: "center" }}>
                              <CategoryIcon sx={{ fontSize: 14, mr: 0.5 }} /> {item.product?.volume || 'N/A'}
                            </Typography>
                          </Box>
                          <Typography variant="h6" sx={{ color: "#F72585", fontWeight: 800 }}>
                            {formatPrice(priceWithTax)}
                            {taxRegime !== 'simplified' && (
                              <Typography component="span" variant="caption" sx={{ ml: 1, opacity: 0.6 }}>IVA incl.</Typography>
                            )}
                          </Typography>
                        </Box>

                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, background: "rgba(255,255,255,0.05)", borderRadius: "12px", p: 0.5 }}>
                          <IconButton onClick={() => handleQuantityChange(item, 'decrement')} disabled={item.quantity <= 1} sx={{ color: "white", "&:hover": { background: "rgba(255,255,255,0.1)" } }}>
                            <RemoveIcon fontSize="small" />
                          </IconButton>
                          <Typography sx={{ minWidth: "2ch", textAlign: 'center', fontWeight: 800, fontSize: '1.2rem' }}>
                            {item.quantity}
                          </Typography>
                          <IconButton onClick={() => handleQuantityChange(item, 'increment')} disabled={item.quantity >= totalAvailableStock} sx={{ color: "white", "&:hover": { background: "rgba(255,255,255,0.1)" } }}>
                            <AddIcon fontSize="small" />
                          </IconButton>
                        </Box>

                        <Box sx={{ minWidth: 100, textAlign: { xs: 'center', sm: 'right' } }}>
                          ...
                          <Button
                            ...
                            color="error"
                            size="small"
                            sx={{ mt: 1, textTransform: "none", opacity: 0.7, "&:hover": { opacity: 1 } }}
                          >
                            Eliminar
                          </Button>
                        </Box>
            ...
            <Grid item xs={12} md={4} className="animate-fade-in-up" sx={{ animationDelay: "0.2s" }}>
              <Card sx={{ ...glassStyle, position: "sticky", top: 20 }}>
                <CardContent sx={{ p: 4 }}>
                  <Typography variant="h5" gutterBottom sx={{ fontWeight: 800, mb: 4 }}>Resumen de Compra</Typography>
                  <Box sx={{ display: "flex", flexDirection: "column", gap: 2, mb: 4 }}>
                    <Box display="flex" justifyContent="space-between">
                      <Typography sx={{ opacity: 0.7 }}>{taxRegime === 'simplified' ? 'Subtotal' : 'Subtotal bruto'}:</Typography>
                      <Typography sx={{ fontWeight: 700 }}>{formatPrice(totalCartPrice)}</Typography>
                    </Box>
                    <Box display="flex" justifyContent="space-between">
                      <Typography sx={{ opacity: 0.7 }}>Envío:</Typography>
                      <Typography variant="body2" sx={{ fontStyle: "italic", opacity: 0.5 }}>Por calcular</Typography>
                    </Box>
                  </Box>
                  ...
                  <Box display="flex" justifyContent="space-between" mb={4}>
                    ...
                    <Typography variant="h5" sx={{ fontWeight: 900, color: "white" }}>{formatPrice(totalCartPrice)}</Typography>
                  </Box>
                  ...
                  <Button
                    fullWidth
                    onClick={() => navigate('/products')}
                    sx={{ mt: 2, color: "white", opacity: 0.6, textTransform: "none", "&:hover": { opacity: 1 } }}
                  >
                    Continuar Comprando
                  </Button>
                </CardContent>
      ...
      >
        ...
        <DialogContent>
          <Typography sx={{ color: "text.secondary" }}>¿Estás seguro de que deseas eliminar todos los productos de tu carrito?</Typography>
        </DialogContent>
        <DialogActions sx={{ p: 3 }}>
          <Button onClick={() => setConfirmClearCartOpen(false)} sx={{ fontWeight: 700, color: "text.secondary" }}>
            Cancelar
          </Button>
          <Button onClick={confirmClearCart} color="error" variant="contained" sx={{ borderRadius: "12px", px: 3, fontWeight: 700 }}>
            Vaciar Todo
          </Button>
        </DialogActions>
      </Dialog>
