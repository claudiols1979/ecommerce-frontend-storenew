...
const ProductCard = ({ product, onAddToCart, isAdding }) => {
  ...
  const priceWithTax =
    displayPrice !== null
      ? taxRegime === "simplified"
        ...
        : calculatePriceWithTax(displayPrice, product.iva)
      : null;
  ...
  const originalPrice = React.useMemo(() => {
    if (
      ...
      product.promotionalLabels.length === 0
    ) {
      return null;
    }

    const discountLabel = product.promotionalLabels.find((label) =>
      label.name.includes("% OFF"),
    );
      ...
      {product.promotionalLabels && product.promotionalLabels.length > 0 && (
        <Box
          sx={{
            ...
            width: "150px",
            py: 0.5,
            background: `linear-gradient(45deg, ${theme.palette.secondary.main} 30%, #ff00d9 90%)`,
          ...
          <Typography
            ...
            sx={{
              ...
              fontSize: [
                ...
                "15% OFF",
                "20% OFF",
              ].includes(product.promotionalLabels[0].name)
                ? "0.55rem"
                : "0.7rem",
            }}
          >
          ...
          >
            <span>
              <Button
                ...
                disabled={
                  isAdding || isOutOfStock || !displayPrice || displayPrice <= 0
                }
                sx={{
                  borderRadius: 2,
                  textTransform: "none",
